...
def probe_dit_checkpoint(path: Path) -> dict[str, Any] | None:
    # Tiny placeholder files are common in unit tests and cannot even hold the
    # 8-byte safetensors header length. Real checkpoints are always much larger;
    # anything bigger that fails to parse is reported as an invalid checkpoint.
    if path.suffix.lower() != ".safetensors" or path.stat().st_size < 8:
        return None
...
def read_network_num_blocks(path: Path) -> int | None:
    if path.suffix.lower() != ".safetensors" or not path.is_file() or path.stat().st_size < 8:
        return None
    from safetensors import safe_open
